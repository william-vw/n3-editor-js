...
exports.exec = async function(options, data, query) {    
    ...
    const [ spin_output, error ] = await promiseExec(cmd);
    // if (error)
    //     throw error
    ...
    let eyeCmd = `${eyeExec} ${folder}/auxiliary-files/runtime.n3 ${data} ${n3_query_file} --query ${n3_filter_file} --nope`
    // switch (subTask) {
    //     case 'derivations':
    ...
    return { 
        ...
        spin: "out/query.spin",
        n3: "out/n3query.n3",
        filter: (filter_file ? 'out/filter.spin' : undefined)
    }
}

async function generateN3(input_file, output_file, label, times) {
    let cmd = `${eyeExec} ${input_file} ${folder}/auxiliary-files/aux.n3 --query ${folder}/queries/query_general.n3 --nope --quantify http://eyereasoner.github.io/.well-known/genid/ > ${output_file};`
    let start = performance.now()

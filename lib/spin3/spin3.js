...
async function generateN3(input_file, output_file, label, times) {
    let cmd = `eye ${input_file} ${folder}/auxiliary-files/aux.n3 --query ${folder}/queries/query_general.n3 --nope --quantify http://eyereasoner.github.io/.well-known/genid/ > ${output_file};`
    let start = performance.now()
